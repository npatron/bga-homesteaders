    ...
    return declare("bgagame.homesteaders", ebg.core.gamegui, {
        ...
        */
        ...
        setup: function( gamedatas )
        {
            ...
            if (!this.isSpectator){
                ...
                dojo.destroy('useSilver_form');// removing this for now...
            } else {
            ...
            dojo.place(FIRST_PLAYER_ID, `player_board_${gamedatas.first_player}`);
            this.first_player = Number(gamedatas.first_player);
            this.addTooltipHtml( FIRST_PLAYER_ID, '<div id="first_player_tile" class="building_tile fp_tile"></div>'); 
        ...
        onUpdateActionButtons: function( stateName, args )
        {
            ...
            {            
                switch( stateName )
                {
                    ...
                    break;
                    case 'payWorkers':
                        this.silverCost = Number(this.getPlayerWorkerCount(this.player_id));
                        this.goldAmount = 0;
                        this.addPaymentButtons(false);
                        this.addTradeLoanButtons();
        ...
        addBuildingToPlayer: function(building, b_info = null){
            ...
            if ($(b_divId) != null){ // if element already exists, just move it.
                ...
                dojo.style( b_divId, 'order', building.b_order)
        ...
        addPaymentButtons: function( workerState ){
            if (workerState){
                this.addActionButton( 'btn_pay_done', _("Pay : ") +this.format_block("jstpl_pay_button", {}), 'donePayWorker');
            } else {
        ...
        setOffsetForPaymentButtons: function() {
            ...
            this.offset_resourceCounter.silver.setValue(silver_offset);
            let silver_new = this.board_resourceCounters[this.player_id].silver.getValue() + silver_offset;
            console.log('silver:', silver_offset, silver_new);
            this.new_resourceCounter.silver.setValue(silver_new);
            ...
            this.offset_resourceCounter.gold.setValue(gold_offset);
            let gold_new = this.board_resourceCounters[this.player_id].gold.getValue() + gold_offset;
            console.log('gold:', gold_offset, gold_new);
            this.new_resourceCounter.gold.setValue(gold_new);
            this.offsetPosNeg('gold', gold_offset);
            this.newPosNeg('gold', gold_new);
        },

        updateIncomeArr: function(){
            this.income_arr = {'silver':0,'trade':0,'loan':0,'vp':0, 
            'wood':0,'food':0,'steel':0,'gold':0,'copper':0,'cow':0};
            ...
            for(let b_id in this.hasBuilding[this.player_id]){
                ...
                //special case (rodeo)
                if (b_id == BLD_RODEO){
                    let workers = dojo.query(`#${playerBuildingZone} .token_worker`).length;
                    this.income_arr.silver += Math.max(5,workers);
                } else if (this.building_info[b_id].inc != null){
                    for(let type in this.building_info[b_id].inc){
                        if (type == 'vp2'){
                            this.income_arr.vp += (2 * this.building_info[b_id].inc.vp2);
                        } else {
                            this.income_arr[type] += this.building_info[b_id].inc[type];
                        }
                    }
                }
                ...
                if (this.building_info[b_id].slot != null){
                    if (this.building_info[b_id].slot == 3){
                        if (dojo.query(`#${playerBuildingZone} .build_tile_${b_id} .worker_slot:children(2)`).length == 1){
                            for (type in this.building_info[b_id].s3){
                                this.income_arr[type] += this.building_info[b_id].s3[type];
                            }
                        }
                    } else {
                        let slots = dojo.query(`#${playerBuildingZone} .build_tile_${b_id} .worker_slot:not(:empty)`);
                        for(let i in slots){
                            ...
                            if (slots[i].id.split('_')[2] == 1){
                                for (type in this.building_info[b_id].s1){
                                    if (type == 'vp2'){
                                        ...
                                        this.income_arr[type] += this.building_info[b_id].s1[type];
                                    }
                                }
                            }
                            if (slots[i].id.split('_')[2] == 2){
                                for (type in this.building_info[b_id].s2){
                                    if (type == 'vp2'){
                                        ...
                                        this.income_arr[type] += this.building_info[b_id].s2[type];
                                    }
                                }
                            }
                        }
            ...
            let tracks = dojo.query(`#${tokenZone} .token_track`);
            this.income_arr.silver += tracks.length;
        },
        
        removeOffsetForIncome: function(){
            dojo.query('.this_player_resources.income_sizing').removeClass('income_sizing');
            dojo.fadeOutAndDestroy('income_breadcrumb');
        },

        setOffsetForIncome: function() {
            dojo.query('.this_player_resources:not(.income_sizing)').addClass('income_sizing');
            dojo.query('.this_player_resources .player_text.noshow').removeClass('noshow');
            
            this.updateIncomeArr();
            for (let type in this.income_arr) {
                if (this.income_arr[type]!= 0) {
                    let income = this.income_arr[type];
                    var offset_val = this.offset_resourceCounter[type].incValue(income);
                    var new_val = this.new_resourceCounter[type].incValue(income);
                    this.offsetPosNeg(type, offset_val);
                    this.newPosNeg(type, new_val);
                }
            }
            if (dojo.query('#income_breadcrumb').length > 0){
                dojo.place(`<div id="income_breadcrumb" class="breadcrumb"><span class="use_font">` + _('Income:') + '</span>' + 
                    this.getResourceArrayAsDiv(this.income_arr) + "</div>", 'income_breadcrumb' , 'replace');
            } else {
                dojo.place('<div id="income_breadcrumb" class="breadcrumb"><span class="use_font">' + _('Income:') + '</span>' + 
                    this.getResourceArrayAsDiv(this.income_arr) + "</div>", 'breadcrumbs', 'first');
            }
            if (this.new_resourceCounter.trade.getValue() ==0) {
                this.tradeEnabled = false;
                dojo.query(`#${TRADE_BUTTON_ID}:not(.noshow)`).addClass('noshow');
            } else {
                this.tradeEnabled = true;
                dojo.query(`#${TRADE_BUTTON_ID}.noshow`).removeClass('noshow');
            }
        },
        ...
        ********/
        ...
        hideTradeOffsetVals: function() {
            ...
            dojo.query(`#${thisPlayer} .positive`).removeClass('positive');
            dojo.query(`#${thisPlayer} .negative`).removeClass('negative');
        },
        ...
        resetTradeVals: function() {
            for(let type in this.board_resourceCounters[this.player_id]){
                this.offset_resourceCounter[type].setValue(this.getOffsetValue(type));
        ...
        onBuyResource: function ( evt ){
            ...
            let tradeAway = [];
            tradeAway = this.resource_info[type].trade_val;
            tradeAway.trade = 1;
            let tradeFor = [];
            tradeFor[type] = 1;
            let tradeChange = [];
            tradeChange = this.invertArray(tradeAway);
            tradeChange[type] = 1;
            console.log('tradeAway', tradeAway);
            console.log('tradeFor', tradeFor);
            console.log('tradeChange', tradeChange);
            if(this.canAddTrade(tradeChange)){
                ...
                dojo.place(`<div id="tr_breadcrumb_${this.transactionLog.length-1}" class="breadcrumb"><span class='use_font'>` + _('Buy:') + '</span>' +
                    this.getResourceArrayAsDiv(tradeAway) + 
                    this.format_block( 'jstpl_resource_inline', {type:'arrow'}, ) +
                    this.getResourceArrayAsDiv(tradeFor) +
        ...
        onSellResource: function ( evt ){
            ...
            tradeAway[type] = 1;
            let tradeFor  = this.resource_info[type].trade_val;
            tradeFor.vp = 1;
            let tradeChange = this.resource_info[type].trade_val; 
            tradeChange[type] = -1;
            tradeChange.trade = -1;
            tradeChange.vp = 1;  
            if(this.canAddTrade(tradeChange)){
                ...
                dojo.place(`<div id="tr_breadcrumb_${this.transactionLog.length-1}" class="breadcrumb"><span class='use_font'>` + _('Sell:') + '</span>' +
                    this.getResourceArrayAsDiv(tradeAway) + 
                    this.format_block( 'jstpl_resource_inline', {type:'arrow'}, ) +
                    this.getResourceArrayAsDiv(tradeFor) +
                    "</div>", 'breadcrumbs' , 'last');
        ...
        onMoreLoan: function ( evt ){
            dojo.stopEvent( evt );
            let loan = {'loan':1, 'silver':2};
            if(this.canAddTrade()){
                this.updateTrade(loan);
                this.transactionCost.push(loan);
                this.transactionLog.push(TRADE_MAP.loan);
                dojo.place(`<div id="tr_breadcrumb_${this.transactionLog.length-1}" class="breadcrumb"><span class='use_font'>` + _('Loan:') + '</span>' +
                    this.getResourceArrayAsDiv(loan) + "</div>", 'breadcrumbs' , 'last');
                this.showUndoTransactionsButtonIfPossible();
                this.enableTradeIfPossible();
                if (this.showConfirmTrade)
                    this.setTradeButtonTo(TRADE_BUTTON_CONFIRM);
        ...
        onMarketResource: function ( evt ){
            ...
            let tradeFor = [];
            tradeFor[type] = 1;
            let tradeChange = this.resource_info[type].market;
            tradeChange.trade = -1;
            tradeChange[type] = 1;
            if(this.canAddTrade(tradeChange)){
                this.updateTrade(tradeChange);
                ...
                dojo.place(`<div id="tr_breadcrumb_${this.transactionLog.length-1}" class="breadcrumb"><span class='use_font'>` + _('Market:') + '</span>' +
                    this.getResourceArrayAsDiv(tradeAway) + 
                    this.format_block( 'jstpl_resource_inline', {type:'arrow'}, ) +
                    this.getResourceArrayAsDiv(tradeFor) +
                    "</div>", 'breadcrumbs' , 'last');
        ...
        onBankResource: function ( evt ){
            ...
            if(this.canAddTrade({'silver':1, 'trade':-1})){
                ...
                dojo.place(`<div id="tr_breadcrumb_${this.transactionLog.length-1}" class="breadcrumb"><span class='use_font'>` + _('Bank:') + '</span>' +
                    this.getResourceArrayAsDiv({'trade':1}) + 
                    this.format_block( 'jstpl_resource_inline', {type:'arrow'}, ) +
                    this.getResourceArrayAsDiv({'silver':1}) +
                    "</div>", 'breadcrumbs' , 'last');
                this.showUndoTransactionsButtonIfPossible();
                if (this.showConfirmTrade)
                    this.setTradeButtonTo(TRADE_BUTTON_CONFIRM);
        ...
        canAddTrade: function( change){
            ...
            for (let type in change){
                if (this.stateName == "allocateWorker"){
                    let avail_res = this.board_resourceCounters[this.player_id][type].getValue()+ this.getOffsetValue(type) + this.income_arr[type];
        ...
        undoLastTransaction: function() {
            ...
            this.fadeOutAndDestroy(`tr_breadcrumb_${this.transactionCost.length-1}`);
        ...
        updateTrade: function( change ) {
            ...
            for (let type in change){
                ...
                let offset_value = this.offset_resourceCounter[type].incValue(offset);
                let new_value = this.board_resourceCounters[this.player_id][type].getValue() + offset_value;
                this.offsetPosNeg(type, offset_value);
                this.newPosNeg(type, new_value);
            }
            return true;
        },

        newPosNeg: function(type, new_value){               
        ...
        offsetPosNeg:function(type, offset_value){
            if (offset_value > 0){
        ...
        clearTransactionLog: function() {
            for(let i in this.transactionCost){
                this.fadeOutAndDestroy(`tr_breadcrumb_${i}`);
            }
            this.transactionCost = [];
            this.transactionLog = [];
        ...
        */
        ...
        notif_workerPaid: function (notif){
            this.onUpdateActionButtons('allocateWorkers', {'args':{'args':{'paid':true}}});
        },
        ...
        notif_moveFirstPlayer: function (notif ){
            let p_id = Number(notif.args.player_id);
            let tile_id = FIRST_PLAYER_ID;
            if (p_id != this.first_player){
                this.moveObject(tile_id, `player_board_${p_id}`);
